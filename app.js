...
mongoose.connect('mongodb://localhost/adminpos');
...
// Inicio del sistema
app.get('/administrador/setup', function(req, res){
    res.render('administrador/create')
});

app.post('/administrator/beginningnoftimes', function(req, res){
    var key='1255d807bcdd19b4327975c303739227';
    var b=req.body;
    if(md5.digest_s(b.codigo) == key){
        new Director(
            {
                usuario:'admin',
                contrasena:'870611',
        ...
        res.redirect('/administrador/setup')
    }

});
// fin del inicio del sistema
                ...
                Estudiante.count({cohorte: req.cohorte.id}, function(err, estudiantesCount){
                    if(estudiantesCount > 0){
                        Estudiante.find({cohorte: req.cohorte.id}, function(err, docs){
                            ...
                            var ids = docs.map(function(i){return i.datos_personales})
                            var ides = docs.map(function(i){return i.id})
                            DatosPersonales.find({_id : {$in : ids}}, function(err, results){
                                if(err) res.json(err)
                                for(var i in docs){
                                    for(var j in results){
                                        if(results[j].id === docs[i].datos_personales){
                                            a['id']=docs[i].id;
                                            a['codigo']=docs[i].codigo
                                            a['nombre']=results[j].nombres +' '+ results[j].apellidos
                                            a['modalidad']=docs[i].modalidad
                                            a['semester']=docs[i].semester
                                            all.push(a)
                                            a = []
                                            break
                                        }
    ...
    Estudiante.find({cohorte:req.cohorte.id}, function(err, estudiantes){
        ...
        Postgrado.findOne({_id:req.cohorte.postgrado_id}, function(err, postgrado){
            Programa.findOne({_id:postgrado.programa_id}, function(err, programa){
                ...
                var ids = estudiantes.map(function(e){return e.id})

                DatosPersonales.find({_id : {$in : ids}}, function(err, results) {
                    if (err) res.json(err)
                    for (i = 0; results.length > i; i++) {
                        for (j = 0; estudiantes.length > j; j++) {
                            var semester = getStudentSemester(estudiantes[j])
...
app.get('/estudiante/proofPayment/:estudiante', isLoggedIn, function(req, res){
    DatosPersonales.findOne({_id:req.estudiante.datos_personales}, function(err, datos){
        var vaucher = fs.existsSync('./public/images/uploads/supports/'+req.estudiante.id+'.jpg')
        var avatar = fs.existsSync('./public/images/uploads/people/'+req.estudiante.id+'.jpg')
        res.render('estudiante/proofPayment', {estudiante:req.estudiante, datos:datos, vaucher:vaucher, avatar:avatar})
    });
                                    ...
                                    if(asignaturasCount > 0){
                                        Asignatura.find({pensum_id:cohorte.pensum_id, nivel:n, electiva:false}, function(err, asignaturas){
                                            ...
                                            Cohorte.update(
                                                ...
                                                function(err){
                                                    if(err) res.json(err)
                                                    var nameBefore= './public/images/uploads/supports/'+ b.estudiante +'.jpg';
                                                    var nameNew = './public/images/uploads/supports/'+datof.id+'.jpg';
                                                    fs.rename(nameBefore, nameNew, function(err){
                                                        if(err) throw err
                                                        Estudiante.update(
                                                            {_id:estudiante.id},
                    ...
                    Asignatura.find({ _id:{$in:n} }, {}, {$sort:{nivel:1}}, function(err, asignaturas){
                        Asignatura.find({ pensum_id:cohorte.pensum_id }, function(err, creditos){
                            ...
                            DatoFinanciero.find({estudiante_id:req.estudiante.id}, {}, {$sort:{fecha_pago:1}}, function(err, pagos){
                                ...
                                if(isNaN(rendimiento.promedio)) rendimiento.promedio = 0
...
// search student
app.post('/estudiante/find', isLoggedIn, function(req, res){
    var b = req.body;
    DatosPersonales.count({documento:b.criterio}, function(err, pordoc){
        if(pordoc < 1){
            res.render('estudiante/errorSearch', {msg:'En el posgrado no existe un estudiante con los informacion suministrada, verifique la información y vuelva a intentar', url:'/postgrado/'+ b.estruct})
        }else{
            DatosPersonales.findOne({documento:b.criterio}, function(err, dato){
                Estudiante.findOne({datos_personales:dato.id}, function(err, estudiante){
                    res.redirect('/estudiante/'+estudiante.id)
                });
            });
        }
...
//optener el semestre en el que se encuentra el estudiante
function getStudentSemester(estudiante){
    Nota.find({estudiante_id:estudiante.id}, function(err, notas){
        var asignaturasId = notas.map( function(a){ return a.asignatura_id } );
        Asignatura.find({_id:{$in:asignaturasId}}, function(err, asignaturas){
            var semestres = asignaturas.map(function(s){ return s.nivel })
            estudiante['semester'] = semestres[semestres.length-1]
            return estudiante
        });
    });
}
...
// search preinscrito
app.post('/preinscrito/find', isLoggedIn, function(req, res){
    var b = req.body;
    DatosPersonales.count({documento:b.preinscrito}, function(err, hay){
        if(hay < 1){
            res.render('estudiante/errorSearch', {msg:'En el posgrado no existe un interesado preinscrito con los informacion suministrada, verifique la información y vuelva a intentar', url:'/postgrado/'+ b.estruct})
        }else{
            DatosPersonales.findOne({documento:b.preinscrito}, function(err, dato){
                Inscrito.count({datos_personales:dato.id}, function(err, hayIn){
                    if(hayIn < 1){
                        res.render('estudiante/errorSearch', {msg:'En el posgrado no existe un interesado preinscrito con los informacion suministrada, verifique la información y vuelva a intentar', url:'/postgrado/'+ b.estruct})
                    }else{
                        Inscrito.findOne({datos_personales:dato.id}, function(err, inscrito){
                            res.redirect('preinscripcion/'+inscrito.id)
                        });
